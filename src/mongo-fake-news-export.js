...
const { FakeNew } = require('./mongoose-module');
...
async function writeInChunks() {
    ...
    try {
        ...
        const totalCount = await FakeNew.countDocuments(dateFilter);
        const totalChunks = Math.ceil(totalCount / CHUNK_SIZE);
        let processedCount = 0;

        for (let chunkNumber = 1; chunkNumber <= totalChunks; chunkNumber++) {
            const chunkPath = `datasets/fake-news/fake-news-${currentDate}-${chunkNumber}.json`;
            ...
            const chunkData = await FakeNew.find(dateFilter).skip(offset).limit(CHUNK_SIZE).cursor();
            const writeData = [];
